...
const ActionButton = styled(motion.a)(({ variant }) => ({
  background:
    variant === 'primary'
    ...
    >
      ...
      <ImageContainer>
        <ProjectImage
          src={url}
          alt={title}
          style={{ filter: isHovered ? 'blur(2px) brightness(0.7)' : 'none' }}
        />
        <OverlayContainer
      ...
      <ContentContainer>
        ...
        <Typography
          variant="body2"
          sx={{ color: '#666', lineHeight: 1.6, fontSize: '14px', flex: 1, marginBottom: '20px' }}
        >
          {description}
        </Typography>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: 'auto' }}>
          {techStack?.map((tech, index) => (
            <TechChip key={index} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              {tech}
            </TechChip>
          ))}
        </Box>
